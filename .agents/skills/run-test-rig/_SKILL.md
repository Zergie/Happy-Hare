...
## Scripts
- **Export code (primary)**: `.agents/skills/run-test-rig/export_to_test_rig.py` — copies Happy-Hare extras and printer_data config to the rig. Run before first test of a session or after code changes.
- **Run test (primary)**: `.agents/skills/run-test-rig/invoke_test_rig.py` — reads GCode from `.agents/skills/run-test-rig/startup.gcode`, injects it into rig `printer.cfg`, auto-prepends `SET_KINEMATIC_POSITION X=150 Y=150 Z=25 SET_HOMED=XYZ`, `MMU_TEST_CONFIG LOG_LEVEL=4`, and `MMU_SELECT GATE=0` before the startup GCode; starts klippy, waits `-DurationSeconds`, kills it, downloads `klippy.log` to `.agents/skills/run-test-rig/klippy.log`.
- **Download log only (primary)**: `.agents/skills/run-test-rig/download_klippy_log.py` — downloads current klippy log from Moonraker and overwrites local `.agents/skills/run-test-rig/klippy.log` without running a scenario.
- **Compatibility wrappers**: `.agents/skills/run-test-rig/Export-ToTestRig.ps1` and `.agents/skills/run-test-rig/Invoke-TestRig.ps1` call Python scripts above.
...
- Run (primary): `python .agents/skills/run-test-rig/export_to_test_rig.py`
- Run (compat): `. .agents/skills/run-test-rig/Export-ToTestRig.ps1`
...
- Write GCode to `.agents/skills/run-test-rig/startup.gcode` (one command per line).
...
- Run (primary): `python .agents/skills/run-test-rig/invoke_test_rig.py -DurationSeconds <N>`
- Run (compat): `. .agents/skills/run-test-rig/Invoke-TestRig.ps1 -DurationSeconds <N>`
...
- Inspect `.agents/skills/run-test-rig/klippy.log`.
- Validate evidence with explicit checks (PowerShell examples):
  `Select-String -Path .agents/skills/run-test-rig/klippy.log -SimpleMatch '<expected pattern>'`
  `Select-String -Path .agents/skills/run-test-rig/klippy.log -SimpleMatch 'Unknown command:','Traceback (most recent call last)'`
...
- Export code (primary): `python .agents/skills/run-test-rig/export_to_test_rig.py`
- Export code (compat): `. .agents/skills/run-test-rig/Export-ToTestRig.ps1`
- Write test GCode to `.agents/skills/run-test-rig/startup.gcode` (one GCode command per line)
- Run scenario (primary): `python .agents/skills/run-test-rig/invoke_test_rig.py -DurationSeconds <N>`
- Run scenario (compat): `. .agents/skills/run-test-rig/Invoke-TestRig.ps1 -DurationSeconds <N>`
- Download log only (primary): `python .agents/skills/run-test-rig/download_klippy_log.py`
- Verify evidence: `Select-String -Path .agents/skills/run-test-rig/klippy.log -SimpleMatch '<expected pattern>'`
- Verify no fatal errors: `Select-String -Path .agents/skills/run-test-rig/klippy.log -SimpleMatch 'Unknown command:','Traceback (most recent call last)'`
- Log location: `.agents/skills/run-test-rig/klippy.log` (downloaded automatically by `invoke_test_rig.py`)
