...
#
# Goal: Provide deterministic BLDC motor, MCU and reactor test doubles.
#
#
# (\_/)
# ( *,*)
...
def install_output_pin_module(repo_root):
    # Use the same pinned queue on developer machines and clean CI checkouts.
    module_path = os.path.join(repo_root, "test", "support", "klipper_output_pin.py")
...
def load_mmu_gear_bldc_module(repo_root):
    ...
    mmu_utils = types.ModuleType("extras.mmu.mmu_utils")

    class MmuError(Exception):
        pass

    mmu_utils.MmuError = MmuError
    sys.modules["extras.mmu.mmu_utils"] = mmu_utils
