...
```
Happy Hare tests - 1339 tests in 47 files        times from last run (~ = reference, never run locally)

   1 [x] installer.test_build           3     1.9s
   2 [x] installer.test_htlf            2     0.7s
   …
  20 [x] test_mmu_console             205      31s
   …
  31 [x] test_mmu_nfc                  29     1.5s
   …
...
full run: `test_mmu_console` took 31 s for 205 tests, `test_mmu_config` 20 s for 35, and
`test_mmu_profiles` 7.7 s for 43, while several files — including
`test_mmu_tag_parser`'s 36 tests — came in under a tenth of a second. A handful of the 47
...
(see that file's own comment) rather than from the more variable CI runner timings.
...
|---|---:|---|
| **Foundation** | | |
| `test_mmu_import.py` | 12 | Happy Hare imports at all outside Klipper; repo-wide syntax check |
| `test_mmu_config.py` | 35 | the real shipped `config/` templates render correctly |
| `test_mmu_reactor.py` | 24 | the fake reactor itself (see §3) |
| `test_mmu_bootup.py` | 36 | config load → `klippy:connect` → `klippy:ready` → `mmu:bootup` |
| `test_mmu_profiles.py` | 43 | the same checks across the bootable machine profiles |
...
| **NFC and Spoolman** | | |
| `test_mmu_nfc.py` | 29 | NFC readers are configured and instantiated |
| `test_mmu_nfc_scan.py` | 54 | `MMU_NFC_SCAN`, the preload NFC compound endstop, the homing presence probe |
| `test_mmu_nfc_i2c.py` | 20 | software (bit-banged) i2c for PN532/PN7160, bus-collision validation |
| `test_mmu_nfc_probe.py` | 47 | the non-blocking presence probe, driver-level (real RC522 over a scripted bus) |
| `test_mmu_nfc_uart.py` | 80 | PN532 over HSU/UART: the byte-stream framer, the probe invariants, the `interface` option |
| `test_mmu_compound_endstop.py` | 16 | which child stopped a first-wins compound home (pure logic) |
| `test_mmu_tag_parser.py` | 36 | RFID tag decoding (pure logic, no fakes at all) |
| `test_mmu_moonraker.py` | 61 | the Moonraker half: Spoolman lookups, auto-create |
| `test_mmu_roundtrip.py` | 54 | Klipper and Moonraker talking to each other |
| **Presentation** | | |
| `test_mmu_leds.py` | 34 | LED effects, flashes, the pending overlay |
| `test_mmu_console.py` | 205 | the interactive console of §1a — rendering, command dispatch |
| `test_mmu_dev_test.py` | 25 | every `_MMU_TEST` developer probe — breadth, not depth |
...
| Area | State | Notes |
|---|---|---|
| Config rendering and load | **solid** | real templates and the bootable shipped machine/unit profiles |
